...
class UserLeaf:
    def __init__(self, pubkey1, pubkey2, balance, nonce, token):
        self.pubkey_1 = pubkey1
        self.pubkey_2 = pubkey2
        self.balance = balance
        self.nonce = nonce
        self.token = token
    def hash_self(self, pubkey1):
        if pubkey1 == self.pubkey_1:
            transaction_string = json.dumps(data.__dict__, sort_keys=True)
            self.account_hash = SHA.new(transaction_string.encode()).hexdigest()
...
//for this class the tree structure were building is an account root, with each  node containging individual account hashes which contain leaf nodes holding public kay, and account transaction infromation
class Usertree:
    """Usertree: this class is currently practically the transaction tree class, needs work to better firt zk transaction structure
        Attributes:
    ...
    def hash_multiple_accounts(self, transactions):
