...
import binascii
from .transaction import Transaction, Transaction_data
...
class User:
    ...
    def create_transaction(self, transaction_data:Transaction_data ):
        if (self.check_transaction(transaction_data) != True):
            return ValueError
        new_transaction = Transaction(self.public_key, transaction_data.recipient, transaction_data.value, transaction_data.coin)
    ...
    def check_transaction(self, transaction_data):
        ...
        if(self.wallet.search_for_coin(transaction_data.coin) == -1):
            ...
            if(self.wallet.coins[self.wallet.search_for_coin(transaction_data.coin)].value < transaction_data.value):
                return ValueError
        return True
